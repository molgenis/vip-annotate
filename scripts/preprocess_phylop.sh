...
usage() {
  echo -e "usage: ${SCRIPT_NAME} -i <arg> -o <arg> [-f]
  -i, --input  <arg>    Input hg38.phyloP100way.bw path
  -o, --output <arg>    Output hg38.phyloP100way.bed.gz path
...
validate() {
  ...
  # validate bgzip available
  if ! command -v bgzip &>/dev/null; then
    echo "error: 'bgzip' could not be found"
    exit 2
  fi

  # validate tabix available

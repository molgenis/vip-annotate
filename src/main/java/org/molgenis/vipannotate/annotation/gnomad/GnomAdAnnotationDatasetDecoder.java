...
public class GnomAdAnnotationDatasetDecoder {
  ...
  public Source decodeSource(MemoryBuffer memoryBuffer, int sourceIndex) {
    ...
    int encodedSourceBatch = memoryBuffer.getByteAtIndex(sourceIndex / nrAnnotationsPerByte);
  ...
  public EnumSet<GnomAdAnnotation.Filter> decodeFilters(
    ...
    int encodedFiltersBatch = memoryBuffer.getByteAtIndex(sourceIndex / nrAnnotationsPerByte);
    int encodedFilters =
  ...
  private double decodeQuantized16UnitIntervalDoublePrimitive(
    ...
    short value = memoryBuffer.getShortAtIndex(afIndex);

...
public record VcfMetaInfo(List<Line> lines) {
  ...
  /**
   * Add or update a meta-information line
   *
   * @return {@code true} if a meta-information line with the given id was added instead of updated
   */
  public boolean addOrUpdateInfo(
      String id,
      String number,
      String type,
      String description,
    ...
    for (int i = 0, linesSize = lines.size(); i < linesSize; i++) {
      ...
      if (aLine instanceof Structured aStructuredLine) {
        if (aStructuredLine.key().equals("INFO") && aStructuredLine.id().equals(id)) {
          String existingSource = aStructuredLine.keyValues().get("SOURCE");
          if ((existingSource == null && source != null)
              || Objects.equals(source, '\"' + existingSource + '\"')) {
            throw new IllegalArgumentException(
                "can't update '%s' because SOURCE '%s' is not '%s'"
                    .formatted(aStructuredLine, existingSource, source));
          }
          existingIndex = i;
          break;
        }
    ...
    if (existingIndex != -1) {
      lines.set(existingIndex, structured);
    } else {
  ...
  public void write(Writer writer) {
    try {
      for (VcfMetaInfo.Line line : lines) {

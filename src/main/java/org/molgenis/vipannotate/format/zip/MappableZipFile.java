...
public class MappableZipFile implements AutoCloseable {
  ...
  public MemorySegment map(ZipArchiveEntry zipArchiveEntry, Arena arena) {
    long dataOffset = zipArchiveEntry.getDataOffset();
    long compressedSize = zipArchiveEntry.getCompressedSize();
    try {
      //noinspection DataFlowIssue
      return this.fileChannel.map(FileChannel.MapMode.READ_ONLY, dataOffset, compressedSize, arena);
    } catch (IOException e) {
